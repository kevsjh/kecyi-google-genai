            ...
            {[4, 3, 2, 1].map((row, rowIndex) => (
              <div key={`row-${rowIndex}`} className="flex flex-row gap-3">
                {['A', 'B', 0, 'C', 'D'].map((seat, seatIndex) => (
                  <div
                    key={`seat-${seatIndex}`}
                    className={`align-center relative flex size-6 flex-row items-center justify-center rounded ${seatIndex === 2
                        ? 'transparent'
                  ...
                  >
                    {seatIndex === 2 ? (
                      ...
                      <div
                        className={`absolute top-0 h-2 w-7 rounded border ${selectedSeat === `${row}${seat}`
